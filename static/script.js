...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateSnowDepthChart = ({
    ...
    season.enter().append('g').merge(season).attr('class', d => `season x${d.season}`).select('path').attr('class', 'line').attr('d', d => line(d.values));
    const currentSeason = seasonContainer.select(`.x${getCurrentSeason()}`).attr('class', 'season current');
    ...
    if (currentSeason.data().length > 0) {
      ...
      d3.select('#comparisonLabel').text(`This date in ${comparisonData.season}`); // need to call raise after raising comparison season
  ...
  const initSnowDepthChart = () => {
    ...
    const xAxis = g.append('g').attr('class', 'axis axis--x').attr('transform', `translate(0,${height - margin.bottom})`).style('font-size', fontSize).call(d3.axisBottom(x).tickFormat(d3.timeFormat('%b')));
    const yAxis = g.append('g').attr('class', 'axis axis--y').attr('transform', `translate(${width - margin.right - margin.left}, 0)`).style('font-size', fontSize).call(d3.axisRight(y)).append('text').attr('transform', 'rotate(-90)').attr('y', -20).attr('dy', '0.71em').attr('x', width > 400 ? -165 : -120).style('fill', '#000').style('font-size', width > 400 ? '14px' : '10px').text('Snow Depth, inches');
    ...
    d3.csv('https://s3.amazonaws.com/matthewparrilla.com/snowDepth.csv', transformRow, csv => {
      ...
      seasonContainer.selectAll('.season').data(data, d => d.season).enter().append('g').attr('class', d => `season x${d.season}`).append('path').attr('class', 'line').attr('d', d => line(d.values));
      ...
      data.map(({
        season
      }) => season).filter(season => season && season !== getCurrentSeason()).sort((a, b) => a < b) // reverse order, so alphabet then reverse 9, 8 etc
      .map(season => ({
        value: season,
        label: season
      })).forEach(season => {
  ...
  const filterToLast10 = data => {
    ...
    .filter(({
      timestamp,
      temperature
    }) => Math.round((today - timestamp) / millisecondsPerDay) < 10 && temperature > -100);
  ...
  const initPrev10Charts = metrics => {
    ...
    const addChart = (data, key, order) => {
      const g = d3.select('#prev_10_charts').select(`#${key}`).attr('transform', `translate(${margin.left},${chartHeight * order + margin.top})`);
      ...
      g.append('text').text(metricLabel[key]).attr('class', `mini_chart_label ${key}`).attr('y', chartHeight - 1.5 * margin.top).attr('dy', '0.71em').style('font-size', fontSize).attr('x', 0);
      line.y(d => y(d[key])).defined(d => d[key] !== null);

      if (key === 'temperature') {
        g.append('path').attr('class', 'line freezing').attr('d', line(data.map(d => ({ ...d,
          temperature: 32
        })))).style('fill-opacity', 0).style('stroke-width', 1.5);
      }

      g.append('g').attr('class', 'y axis').style('font-size', fontSize).attr('transform', `translate(${width - margin.left - margin.right}, 0)`).call(yAxis);
      g.append('path').attr('class', `line prev_10_line ${key}`).attr('d', line(data)).style('fill-opacity', 0).style('stroke-width', 1.5); // Only show tick labels on last row
      ...
      g.append('g').attr('class', 'x axis').attr('transform', `translate(0,${chartHeight})`).style('font-size', fontSize).call(xAxis);
    ...
    d3.csv('https://s3.amazonaws.com/matthewparrilla.com/mansfield_observations.csv', transformDate, unfilteredData => {
      ...
      const mouseG = d3.select('#prev_10_charts').append('g').attr('class', 'mouse-over-effects').attr('transform', `translate(0,${margin.top})`);
      ...
      .attr('height', containerHeight).attr('fill', 'none').attr('pointer-events', 'all').on('mouseout', () => setMouseOverOpacity(0)).on('mouseover', () => setMouseOverOpacity(1)).on('mousemove', function onMouseMove() {
        ...
        d3.select('.mouse-line').attr('d', () => `M${mouseX},${containerHeight} ${mouseX},0`); // find intersection of x position and relevant line
        ...
        d3.selectAll('.mouse-per-line').attr('transform', (key, i) => `translate(${margin.left + x(intersection.timestamp)}, ${metricY[key](intersection[key]) + chartHeight * i})`).selectAll('text.shadow').text(key => intersection[key]);
        d3.selectAll('.mouse-per-line').attr('transform', (key, i) => `translate(${margin.left + x(intersection.timestamp)}, ${metricY[key](intersection[key]) + chartHeight * i})`).selectAll('text.fill').text(key => intersection[key]);
      }); ///////////////////////////////////////////////////
      ////////////// END MOUSEOVER CODE /////////////////
      ///////////////////////////////////////////////////
    });
